...
static esp_err_t capture_handler(httpd_req_t *req) {
  ...
  char ts[32];
  snprintf(ts, 32, "%ld.%06ld", (long)fb->timestamp.tv_sec, (long)fb->timestamp.tv_usec);
  httpd_resp_set_hdr(req, "X-Timestamp", (const char *)ts);
...
// Simple LED control endpoint: /led?on=0/1&duty=0..255
#if defined(LED_GPIO_NUM)
static esp_err_t led_handler(httpd_req_t *req) {
  ...
  if (has_duty) {
    int d = atoi(duty_str);
    if (d < 0) d = 0;
    if (d > CONFIG_LED_MAX_INTENSITY) d = CONFIG_LED_MAX_INTENSITY;
    led_duty = d;
  }
...
static esp_err_t status_handler(httpd_req_t *req) {
  ...
  *p++ = '{';
  ...
  p += sprintf(p, ",\"buzzer_duty\":%u", buzzer_duty);
  *p++ = '}';
  *p++ = 0;
...
// Minimal raw TCP socket streaming server
// Protocol: for cada frame JPEG, se envía uint32_t (big-endian) con el tamaño, seguido del buffer JPEG
// Puerto sugerido: 3333
class RawSocketServer {
public:
  WiFiServer server;
  RawSocketServer(uint16_t port) : server(port) {}
    ...
    while (client.connected()) {
      ...
      if (fb->format != PIXFORMAT_JPEG) {
        ...
        if (frame2jpg(fb, 80, &out, &out_len)) {
          esp_camera_fb_return(fb);
          fb = NULL; jpg_buf = out; jpg_len = out_len;
        }
      }
      // Send 4-byte length (big-endian)
      uint8_t hdr[4];
      hdr[0] = (jpg_len >> 24) & 0xFF;
      hdr[1] = (jpg_len >> 16) & 0xFF;
      hdr[2] = (jpg_len >> 8) & 0xFF;
      hdr[3] = (jpg_len) & 0xFF;
      if (client.write(hdr, 4) != 4) { if (fb) esp_camera_fb_return(fb); break; }
      size_t written = 0;
      while (written < jpg_len) {
        size_t chunk = client.write(jpg_buf + written, min((size_t)1460, jpg_len - written));
        if (!chunk) { break; }
        written += chunk;
      }
      if (fb) esp_camera_fb_return(fb);
      if (written < jpg_len) { break; }
      // Yield a bit
      vTaskDelay(1);
    }
    client.stop();
...
static RawSocketServer *rawSocketServer = nullptr;

void startRawSocketStreamServer(uint16_t port) {
  if (rawSocketServer) return;
  rawSocketServer = new RawSocketServer(port);
  rawSocketServer->begin();
  log_i("Raw socket stream server started on %u", port);
}

// Call periodically from loop()
void handleRawSocketServer() {
  if (rawSocketServer) rawSocketServer->handle();
}
